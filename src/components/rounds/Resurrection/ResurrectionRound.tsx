...
export default function ResurrectionRound({ activeItems, eliminatedItems, onItemResurrection }: Props) {
  ...
  return (
    <>
      {state === "initial" && (
        <Box style={{ justifyContent: "center" }}>
          ...
          <Box marginTop={2}>
            ...
            <Button variant="contained" color="error" onClick={() => setState("voting")} sx={{ marginLeft: 2 }}>
      ...
      {state === "voting" && (
        <Box>
          <ResurrectionVoting items={eliminatedItems} onItemElimination={(id) => onItemResurrection(id)} />
        </Box>
      )}
    </>
  );
